...
ここまでコードが到達すれば、IDがprojectIdの企画のステータスを変更できます。許容されている3つ(OK,Limited,Suspended)であればその状態にし、それ以外であれば状態をなし(null)に設定します。  
Prismaのupdateを使い、対象の企画のデータを更新します。その後、更新後の企画状態を返します。
```js
